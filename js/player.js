...
class Player {
  constructor(x, y) {
    ...
    this.jumpForce  = -420; // px/s initial vertical impulse
  ...
  _resolveAABB(rect) {
    const overlapX = Math.min(this.x + this.w - rect.x, rect.x + rect.w - this.x);
    const overlapY = Math.min(this.y + this.h - rect.y, rect.y + rect.h - this.y);

    if (overlapY <= overlapX) {
      // vertical collision
      if (this.y + this.h / 2 < rect.y + rect.h / 2) {
      ...
      if (this.x + this.w / 2 < rect.x + rect.w / 2) {
  ...
  /** Returns the attack hitbox {x,y,w,h} or null if not attacking */
  getAttackHitbox() {
  ...
  draw(ctx, camX) {
    ...
    // Attack hitbox debug (comment out in release)
    // const hb = this.getAttackHitbox();
    // if (hb) { ctx.strokeStyle = PALETTE.primary; ctx.strokeRect(hb.x - camX, hb.y, hb.w, hb.h); }
  }

  /** Pixel-art sprite drawn with rect primitives (32x32 grid, 24x32 actual) */
  _drawSprite(ctx) {
    const p = PALETTE;
    // Body silhouette (#333 outline, #9EFF8B body)
    ctx.fillStyle = p.mid;
    ctx.fillRect(4,  8, 16, 16); // torso
    ctx.fillRect(6,  0, 12, 10); // head
    ctx.fillStyle = p.primary;
    ctx.fillRect(6,  2, 12, 7);  // helmet
    ctx.fillStyle = p.text;
    ctx.fillRect(8,  4, 4,  3);  // visor slit
    // Legs
    ctx.fillStyle = p.mid;
    ctx.fillRect(4,  24, 6, 8);  // left leg
    ctx.fillRect(14, 24, 6, 8);  // right leg
    // Sword arm (if attacking, extend arm right)
    ctx.fillStyle = p.primary;
    if (this.attacking) {
      ctx.fillRect(20, 10, 10, 4); // sword extended
      ctx.fillRect(16,  8,  6, 8); // arm
    } else {
      ctx.fillRect(16, 10, 4, 10); // arm at rest
    }
    ctx.fillRect(0,  10, 4, 10); // shield arm
    ctx.fillStyle = p.text;
    ctx.fillRect(0,   8, 4, 14); // shield
  }
}
